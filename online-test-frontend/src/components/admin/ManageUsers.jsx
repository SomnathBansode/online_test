...
const ManageUsers = () => {
  ...
  const handleActivate = async (user) => {
    try {
      const res = await axios.patch(`/users/${user._id}/activate`, {
        status: user.status === "active" ? "inactive" : "active",
      });
      setUsers(
        users.map((u) =>
          u._id === user._id ? { ...u, status: res.data.status } : u
        )
      );
    } catch (err) {
          ...
          <div className="bg-white dark:bg-[#232e41] rounded-xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleEditSubmit} className="flex flex-col gap-4">
              <input
                ...
                className="py-2 px-3 rounded border border-[#a1724e] dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-base"
                required
              />
              <input
                type="email"
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-[#232e41] rounded-xl p-6 w-full max-w-sm shadow-2xl flex flex-col items-center">
            <h3 className="text-lg font-bold mb-4 text-[#482307] dark:text-green-400">
              {t("Delete User?")}
            </h3>
            <p className="mb-6 text-[#a1724e] dark:text-gray-200 text-center">
              {t(
      ...
      {loading ? (
        ...
        <div className="w-full overflow-x-auto">
            ...
            <tbody>
              {paginatedUsers.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                    className="text-center py-4 text-[#a1724e] dark:text-green-400"
                  >
                    {t("No users found.")}
                  </td>
                </tr>
                  ...
                  >
                    ...
                    <td className="py-2 px-4 align-middle text-right">
                      <div className="inline-flex gap-2">
                        <button
                          className="min-w-[80px] px-3 py-1 rounded-lg bg-gradient-to-r from-[#4f8cff] to-[#1e40af] hover:from-[#2563eb] hover:to-[#1e3a8a] dark:from-[#60a5fa] dark:to-[#1e293b] text-white font-semibold transition shadow-md text-sm"
                          onClick={() => handleEdit(user)}
                        >
                          {t("Edit")}
                        </button>
                        <button
                          className={`min-w-[80px] px-3 py-1 rounded-lg font-semibold shadow-md text-sm ${
                            user.status === "active"
                        ...
                        <button
                          className="min-w-[80px] px-3 py-1 rounded-lg bg-gradient-to-r from-[#ff5e62] to-[#ff9966] hover:from-[#dc2626] hover:to-[#f59e42] dark:from-[#f87171] dark:to-[#fbbf24] text-white font-semibold transition shadow-md text-sm"
          ...
          <div className="flex justify-center items-center gap-2 mt-4">
            ...
            <span className="text-[#482307] dark:text-green-400 font-medium">
