...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || t('Failed to send reset email'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthWrapper>
      <div className="bg-[#fff9ea] dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-xl w-full max-w-md border border-[#a1724e] dark:border-gray-700 mx-auto">
        <h1 className="text-2xl font-bold mb-4 text-center text-[#a1724e] dark:text-green-400">{t('Forgot Password')}</h1>
